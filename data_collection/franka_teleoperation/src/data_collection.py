...
class DataCollection:
    ...
    def franka_subscribers(self):
        ...
        ts = message_filters.ApproximateTimeSynchronizer([fing_cam_sub, front_cam_sub, side_cam_sub, joint_state_sub, task_space_sub], queue_size=10, slop=0.1)
    ...
    def callback(self, fing_cam, front_cam, side_cam, joint_sub, task_sub):
        self.follower_joint_states.append(np.asarray(joint_sub.position))
        self.follower_task_states.append(task_sub.O_T_EE)

        try:
            cv_image = self.bridge.imgmsg_to_cv2(fing_cam, "bgr8")
            cv_image_front = self.bridge.imgmsg_to_cv2(front_cam, "bgr8")
            cv_image_side = self.bridge.imgmsg_to_cv2(side_cam, "bgr8")
        except CvBridgeError as e:
            print("CvBridgeError: ", e)

        self.fing_cam.append(np.asarray(cv_image))
        self.front_cam.append(np.asarray(cv_image_front))
    ...
    def post_processing(self):
        ...
        self.save_to_file("fing_camera_" + sys.argv[1] + ".npy", self.fing_cam)
        self.save_to_file("front_camera_" + sys.argv[1] + ".npy", self.front_cam)
    ...
    def main(self):
        ...
        while not rospy.is_shutdown():
            ...
            if (self.init_front_cam == False) or (self.init_fing_cam == False) or\
               (self.follower_joint_states_arr == False) or (self.follower_task_states_arr == False):
                ...
                self.frames = []
                self.init_fing_cam = True
                self.init_front_cam = True
                ...
                try:
                    ...
                    if self.stop:
                        ...
                        self.frames.append(len(self.fing_cam))
                        frame_rate = len(self.fing_cam) / exp_time
