...
class FrankaRobot(object):
    def __init__(self):
        ...
        # datasaving:
        self.datasave_folder = "/home/gabriele/Dataset/Pushing_Blocks/"
        ...
        self.front_cam_sub = message_filters.Subscriber("/front_camera/color/image_raw", Image)
        self.side_cam_sub = message_filters.Subscriber("/side_camera/color/image_raw", Image)
        self.tilt_cam_sub = message_filters.Subscriber("/tilted_camera/color/image_raw", Image)
        self.ts = message_filters.ApproximateTimeSynchronizer([self.robot_sub, self.fing_cam_sub, self.front_cam_sub, self.side_cam_sub, self.tilt_cam_sub] , queue_size=1, slop=0.1, allow_headerless=True)
    ...
    def pushing_actions(self):
        ...
        for j in range(self.resets):
            for i in range(self.pushes_per_reset):
                ...
                # finish_pose.position.x = finish_x_position
    ...
    def read_robot_data(self, robot_joint_data, fing_cam, front_cam, side_cam, tilt_cam):
        if self.i != self.prev_i:
            ...
            self.camera_tilted.append(tilt_cam)
    ...
    def save_data(self):
        ...
        print("camera_tilted", np.array(self.camera_tilted).shape)
        ...
        np.save(folder + '/camera_tilted.npy', np.array([self.bridge.imgmsg_to_cv2(image, desired_encoding='passthrough') for image in self.camera_tilted]))
